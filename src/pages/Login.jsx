...
export default function Login() {
  const { session, operator, signIn, switchOperator, usuarios, loading } = useAuth();
        ...
        {selUser && (
          <div className="w-full animate-slide-up">
            <p className="mb-3 text-center text-sm font-bold text-muted">
              PIN de {selUser.nombre}
            </p>
            <div className="mb-4 flex justify-center gap-3">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`h-3 w-3 rounded-full ${i < pin.length ? 'bg-bodega' : 'bg-white ring-1 ring-black/10'}`}
                />
              ))}
            </div>
            <div className="grid grid-cols-3 gap-2">
              {['1','2','3','4','5','6','7','8','9','←','0','⌫'].map((k) => (
                <button
                  ...
                  disabled={pinLoading}
                  className={`flex h-14 items-center justify-center rounded-xl bg-white text-lg font-extrabold shadow-sm active:scale-95 ${
                    k === '⌫' ? 'text-red' : 'text-ink'
                  }`}
                >
                  {k}
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-[480px] flex-col items-center justify-center bg-cream px-5">
      ...
      <form onSubmit={handleAuth} className="w-full">
        ...
        />
        <Btn type="submit" disabled={authLoading}>
